...
import { buildUri, getAddresses, getEvents } from './utils';
...
const indexerURI = buildUri(environment || NomadEnvironment);
let addresses = [];
...
export default (): void => {
  ...
  step('Query Get Events', async () => {
    for (let index = 0; index < addresses.length; index++) {
      const address = addresses[index];
      await getEvents(indexerURI, address);
    }
  });
};
