    ...
    async fetch(id: string, domain: number) {
        ...
        try {
            [
                ...
                [totalSupply],
                [balance],
            ] = await new F(token).with('name', 'decimals', 'symbol', 'totalSupply', ['balanceOf', this.sdk.mustGetBridge(domain).bridgeRouter.address, ]).all();
        } catch(e) {
        ...
        if (domain === this.sdk.governor.domain) {
            remotes = this.sdk.domainNumbers.filter(remoteDomain => remoteDomain !== domain);
        } else {
        ...
        await Promise.all(remotes.map(async (remoteDomain) => {
            let remoteId: string;
            try {
                remoteId = await this.sdk.mustGetBridge(remoteDomain).tokenRegistry.getRepresentationAddress(domain, id);
            } catch(e: any) {
            ...
            try {
                [
                    ...
                    [_symbol],
                    [_totalSupply],
                ] = await new F(token).with('name', 'decimals', 'symbol', 'totalSupply', ).all();
            } catch(e) {
...
const tokens: [string,number][] = [
    ['0x0000000000000000000000000bf0d26a527384bcc4072a6e2bca3fc79e49fa2d',6648936],
    ['0x0000000000000000000000002260fac5e5542a773aa44fbcfedf7c193bc2c599',6648936],
    ['0x0000000000000000000000003432b6a60d23ca0dfca7761b7ab56459d9c964d0',6648936],
    ['0x0000000000000000000000006b175474e89094c44da98b954eedeac495271d0f',6648936],
    ['0x000000000000000000000000853d955acef822db058eb8505911ed77f175b99e',6648936],
    ['0x000000000000000000000000a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48',6648936],
    ['0x000000000000000000000000acc15dc74880c9944775448304b263d191c6077f',1650811245],
    ['0x000000000000000000000000c02aaa39b223fe8d0a0e5c4f27ead9083c756cc2',6648936],
    ['0x000000000000000000000000dac17f958d2ee523a2206206994597c13d831ec7',6648936],
    ['0x000000000000000000000000f0dc76c22139ab22618ddfb498be1283254612b1',6648936],
];
