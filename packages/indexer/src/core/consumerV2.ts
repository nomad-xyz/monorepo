...
import { Dispatch, EventType, NomadishEvent, Process, Send, Update } from "./event";
...
export class ProcessorV2 extends Consumer {
    ...
    async checkAndUpdateSend(m: NomadMessage) {
      if (m.msgType !== MessageType.TransferMessage) throw new Error(`Message not a transfer message`);
    ...
    async bridgeRouterSend(e: NomadishEvent) {
      ...
      const block = e.block;
      const {toDomain: destination, toId: recipient, amount} = (e.eventData as Send);
