...
export class HardhatNetwork extends Network {
  ...
  async deployWETH(): Promise<string> {
    ...
    const owner = jsonRpcProvider.getSigner();
...
export class ForkedNetwork extends Network {
  ...
  constructor(name: string, domain: number, docker: Dockerode, forkUrl: string, blockExplorer?: string, blocktime?: number, options?: HardhatNetworkOptions) {
    super(name, domain, domain, docker);
    if (blocktime) {
