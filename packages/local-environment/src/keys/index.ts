...
export interface IAgentKeysOutput {
  ...
  export class AgentKeys {
    ...
    constructor(keys?: IAgentKeysInput) {
      ...
      this.watchers = keys?.watchers.map(w => new Key(w)) || [new Key(DefaultAgentKeys.watcher)];
    }
      ...
      switch (type) {
        case AgentType.Updater:
          ...
          case AgentType.Watcher:
            if (this.watchers.length < watcherNumber + 1 ) throw new Error(`There is no watcher key [${watcherNumber}]`);
