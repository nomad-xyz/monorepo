...
export abstract class AbstractCoreDeploy<T> extends Contracts<T> {
  ...
  // abstract homeRoot(): Promise<string>;
  // abstract governanceRouterAddress(): Promise<string>;
}



export class SubstrateCoreDeploy extends AbstractCoreDeploy<config.SubstrateCoreDeploymentInfo> {
  protected keys: ReadonlyArray<keyof config.SubstrateCoreDeploymentInfo> = [
    'deployHeight',
  ];
  ...
  constructor(
    ...
    domain: string,
    data?: config.SubstrateCoreDeploymentInfo,
  ) {
  ...
  async governanceRouterAddress(): Promise<string> {
    return 'this.home.root()'
  }
}



export class EvmCoreDeploy extends AbstractCoreDeploy<config.EthereumCoreDeploymentInfo> {
  ...
  async homeRoot(): Promise<string> {
    return await this.home.root()
  }

  async governanceRouterAddress(): Promise<string> {
    return this.governanceRouter.address;
  }
  ...
  async deployUnenrolledReplica(remoteDomain: string | number, nonevm?: boolean): Promise<void> {
    const local = this.context.resolveDomainName(this.domain);
    const remote = this.context.resolveDomainName(remoteDomain);
    const localConfig = this.context.mustGetDomainConfig(local);
    const remoteConfig = this.context.mustGetDomainConfig(remote);
    ...
    if (nonevm) {
      ...
      const remoteCore = this.context.mustGetCore(remote);
      root = await remoteCore.home.committedRoot() || defaultRoot;
    }
  ...
  async enrollGovernanceRouter(
    ...
    if (utils.equalIds(enrolledRemote, await remoteCore.governanceRouterAddress()))
      return;
    log(`enroll GovernanceRouter for ${remote} on ${local}`);
    ...
    if (!utils.equalIds(owner, deployer)) {
      const call =
        await this.governanceRouter.populateTransaction.setRouterLocal(
          remoteConfig.domain,
          utils.canonizeId(await remoteCore.governanceRouterAddress()),
        );
      const batch = CallBatch.fromContext(this.context.asNomadContext);
      // safe as populateTransaction always sets `to`
      batch.push(this.domainNumber, call as Call);
  ...
  async enrollRemote(remoteDomain: string | number, nonevm?: boolean): Promise<CallBatch> {
    await this.deployUnenrolledReplica(remoteDomain, nonevm);
    let batches: (CallBatch | undefined)[];
    if (nonevm) {
    ...
    for (const remoteDomain of remoteDomains) {
      ...
      checklist.addCheck({
        ...
        check: async () => {
          ...
          checklist.equalIds(
            await this.context.mustGetCore(remoteDomainNumber).governanceRouterAddress(),
            remoteRouter,
          );
        },
      });
